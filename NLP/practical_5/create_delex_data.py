...
# For example, for the belief state     
# { 'food': 'british', 'pricerange': "don't care", 'name': 'not mentioned', 'area': 'centre'}
# the function should return:
# [0, 0, 1] for slot food
# [0, 1, 0] for slot pricerange
# [1, 0, 0] for slot name
# [0, 0, 1] for slot area
# which combined gives: [0, 0, 1, 0, 1, 0, 1, 0, 0, 0, 0, 1]
def belief_state(bstate, summary_bstate, domain):
    ...
    """
    # YOUR CODE HERE
    bstate_semi = bstate[domain]['semi']
    slots = ['food', 'pricerange', 'name', 'area']

    for slot in slots:
        if bstate_semi[slot] == 'not mentioned':
            summary_bstate.extend([1, 0, 0])
        elif bstate_semi[slot] == "don't care":
            summary_bstate.extend([0, 1, 0])
        else:
            summary_bstate.extend([0, 0, 1])
