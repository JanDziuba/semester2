...
from utils.db_pointer import query_result_venues
...
def evaluateModel(dialogues, val_dials, mode='valid'):
    ...
    gen_stats = {
        'restaurant': [0, 0, 0], 'hotel': [0, 0, 0], 'attraction': [0, 0, 0],
        'train': [0, 0, 0], 'taxi': [0, 0, 0],
    ...
    for filename, dial in list(dialogues.items()):
        ...
        success, match, stats = evaluateGeneratedDialogue(
            dial, goal, data, requestables)
    ...
    # Print results
    if mode == 'valid':
        try: print("Valid BLUES SCORE %.10f" % bscorer.score(
            model_corpus, corpus))
        except: print('BLUE SCORE ERROR')
        print('Valid Corpus Matches : %2.2f%%' % (
            matches / float(total) * 100))
        print('Valid Corpus Success : %2.2f%%' %  (
            successes / float(total) * 100))
        print('Valid Total number of dialogues: %s ' % total)
    else:
        try:
            print("Corpus BLUES SCORE %.10f" % bscorer.score(
                model_corpus, corpus))
        except:
...
def evaluateGeneratedDialogue(dialog, goal, realDialogue, real_requestables):
    ...
    for domain in list(goal.keys()):
        match_stat = 0
        if domain in ['restaurant']:
            goal_venues = query_result_venues(
            ...
            if (type(venue_offered[domain]) is str and
                ...
                venue_offered[domain][0] in goal_venues):
                match += 1
                match_stat = 1
        else:
...
def evaluateCorpus():
    ...
    for dialogue in train_dials:
        ...
        goal, success, match, requestables, stats = evaluateRealDialogue(
            train_dials[dialogue])
        successes += success
        matches += match
        total += 1
