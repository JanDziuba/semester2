...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunc_len = np.random.randint(4, int(self.block_size * 7 / 8) + 1)
        trunc_len = min(trunc_len, len(document))

        trunc_document = document[:trunc_len]
        mask_len = np.random.binomial(trunc_len, 1/4)
        prefix_len = np.random.randint(trunc_len - mask_len + 1)
