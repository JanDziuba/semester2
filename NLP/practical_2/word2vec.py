...
def neg_sampling_loss_and_gradient(
    ...
    loss = - np.log(sigmoid(u_o @ centerWordVec))
    for idx in negSampleWordIndices:
        u_k = outsideVectors[idx]
        loss -= np.log(sigmoid(-u_k @ centerWordVec))

    gradCenterVec = (sigmoid(u_o @ centerWordVec) - 1) * u_o
    for idx in negSampleWordIndices:
        u_k = outsideVectors[idx]
        gradCenterVec += sigmoid(u_k @ centerWordVec) * u_k

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] += (sigmoid(u_o @ centerWordVec) - 1) * centerWordVec
    for idx in negSampleWordIndices:
        u_k = outsideVectors[idx]
        gradOutsideVecs[idx] += sigmoid(u_k @ centerWordVec) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        wordLoss, wordGradCenterVec, wordGradOutsideVecs = word2vecLossAndGradient(currentCenterWordVec,
                                                                                   word2Ind[outsideWord],
                                                                                   outsideVectors, dataset)
        loss += wordLoss
        gradCenterVecs[word2Ind[currentCenterWord]] += wordGradCenterVec
